  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="relative flex h-9 w-9 items-center justify-center rounded-full border border-black/10 text-zinc-600 hover:bg-zinc-100 dark:border-white/10 dark:text-zinc-300 dark:hover:bg-white/10"
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-30 mt-2 w-80 rounded-xl border border-black/10 bg-white p-2 shadow-lg dark:border-white/10 dark:bg-zinc-950">
            <div className="flex items-center justify-between px-2 py-1.5">
              <span className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">{t("notif.title")}</span>
              {unreadCount > 0 && (
                <button
            ...
            {myNotifications.length === 0 ? (
              <p className="px-2 py-6 text-center text-sm text-zinc-400">{t("notif.none")}</p>
            ) : (
                  ...
                  return (
                    <li key={n.id}>
                      <Link
                        ...
                        className={`block rounded-lg px-2 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-white/5 ${
                          n.read ? "text-zinc-500 dark:text-zinc-400" : "font-medium text-zinc-900 dark:text-zinc-50"
                        }`}
                      >
